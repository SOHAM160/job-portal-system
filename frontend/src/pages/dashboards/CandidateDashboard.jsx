...
import { Briefcase, MapPin, DollarSign, Search, Loader2, CheckCircle2, Bookmark, BookmarkCheck, Sliders, FileText, Upload, X, ScanSearch, Calendar, Video, MessageSquare, TrendingUp, BrainCircuit, ChevronRight } from "lucide-react";
import CandidateAnalytics from "../../components/CandidateAnalytics";
import JobRecommendations from "../../components/JobRecommendations";
import toast from "react-hot-toast";
import { getAllJobs, applyToJob, getMyApplications, toggleSavedJob, scanResumeATS, updateProfile } from "../../api";
...
const CandidateDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState("jobs"); // "jobs" | "applications" | "insights" | "matches"
  ...
  const fetchData = async (searchParamsObj = {}) => {
    try {
      const [jobsRes, appsRes] = await Promise.all([getAllJobs(searchParamsObj), getMyApplications()]);
  ...
  // Handle URL search params: 'q' comes from navbar global search, 'keyword/location/etc.' from Filter form
  useEffect(() => {
    const urlQ = searchParams.get("q") || "";

    if (urlQ) {
      // Global navbar search — fetch by q tag/title match, do NOT populate lower filter inputs
      setLoading(true);
      fetchData({ q: urlQ });
    } else {
      // Filter form search — read all filter fields from URL and populate the inputs
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    // Filter form submit: clear any navbar 'q' param and set filter params instead
    const newParams = {};
    if (filters.keyword) newParams.keyword = filters.keyword;
    if (filters.location) newParams.location = filters.location;
    if (filters.skills) newParams.skills = filters.skills;
    if (filters.salary) newParams.salary = filters.salary;
    if (filters.type && filters.type !== "All") newParams.type = filters.type;
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      await updateProfile({ bio: profileData.bio, skills: skillsArray });
      await refreshUser();
      setIsEditingProfile(false);
      toast.success("Profile updated! Matches will now be more accurate.");
    } catch (error) {
  ...
  const handleViewResume = (applicationId) => {
    ...
    const url = `${import.meta.env.VITE_API_URL}/api/applications/${applicationId}/resume`;
  ...
  if (!user) return <div className="flex justify-center p-20"><Loader2 className="animate-spin text-primary-600" /></div>;
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-xl card-shadow border border-slate-200 overflow-hidden sticky top-8">
            ...
            <div className="px-6 pb-6 -mt-8 flex flex-col items-center text-center">
              <div className="w-16 h-16 bg-white rounded-full border-4 border-white shadow-sm flex items-center justify-center font-bold text-xl text-primary-600 mb-3 overflow-hidden">
                {user?.profilePicture ? <img src={user.profilePicture} alt="Profile" className="w-full h-full object-cover" /> : user?.name?.[0] || "U"}
              </div>
              <h3 className="font-bold text-slate-900">{user?.name}</h3>
              <p className="text-xs text-slate-500 font-medium capitalize">Aspiring {user?.role}</p>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <BrainCircuit className="w-3 h-3 text-primary-600" /> Profile Context
                </p>
                <button onClick={() => setIsEditingProfile(!isEditingProfile)} className="text-[10px] font-bold text-primary-600 hover:underline">
              ...
              {isEditingProfile ? (
                <form onSubmit={handleUpdateProfile} className="space-y-3">
                  <textarea
                    ...
                    className="w-full text-xs p-2 rounded border border-slate-200 focus:ring-1 focus:ring-primary-500 h-20"
                  />
                  <input
                    value={profileData.skills}
                    onChange={(e) => setProfileData({ ...profileData, skills: e.target.value })}
                    placeholder="Skills (comma separated)"
                    className="w-full text-xs p-2 rounded border border-slate-200 focus:ring-1 focus:ring-primary-500"
                  />
                  <button type="submit" className="w-full bg-primary-600 text-white text-[10px] font-bold py-2 rounded shadow-sm">Save Profile</button>
                </form>
              ) : (
                <div className="space-y-3">
                  <p className="text-xs text-slate-600 line-clamp-3">{user.bio || "No bio added yet. Add one to improve matches."}</p>
                  <div className="flex flex-wrap gap-1">
                    {user.skills?.length > 0 ? user.skills.slice(0, 5).map((s, i) => (
                      <span key={i} className="text-[9px] bg-white border border-slate-200 px-1.5 py-0.5 rounded text-slate-500 font-bold">{s}</span>
        ...
        <div className="lg:col-span-3 space-y-6">
          {/* Tab Bar */}
          <div className="flex flex-wrap gap-2 bg-white p-1.5 rounded-xl card-shadow border border-slate-200 w-fit">
            <button onClick={() => setActiveTab("jobs")} className={`px-4 md:px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === "jobs" ? "bg-primary-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-100"}`}>
              <Briefcase className="w-4 h-4 inline mr-2" />Browse Jobs
            </button>
            <button onClick={() => setActiveTab("applications")} className={`px-4 md:px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === "applications" ? "bg-primary-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-100"}`}>
              <FileText className="w-4 h-4 inline mr-2" />Applications ({applications.length})
            </button>
            <button onClick={() => setActiveTab("insights")} className={`px-4 md:px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === "insights" ? "bg-primary-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-100"}`}>
              <TrendingUp className="w-4 h-4 inline mr-2" />Insights
            </button>
            <button onClick={() => setActiveTab("matches")} className={`px-4 md:px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === "matches" ? "bg-primary-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-100"}`}>
              <BrainCircuit className="w-4 h-4 inline mr-2" />Quick Matches
            </button>
          </div>

          <div className="min-h-[400px]">
            {/* ── TAB: Insights ────────────────────────────────────── */}
            {activeTab === "insights" && <CandidateAnalytics />}

            {/* ── TAB: Smart Matches ────────────────────────────────── */}
            {activeTab === "matches" && <JobRecommendations />}
            ...
            {activeTab === "applications" && (
              <div className="space-y-4">
                {applications.length > 0 ? applications.map((app) => (
                  <div key={app._id} className="bg-white card-shadow rounded-xl p-6 border border-slate-100 group hover:border-primary-100 transition-all">
                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                      <div className="flex-1">
                        <h3 className="font-bold text-lg text-slate-900 group-hover:text-primary-600 transition-colors uppercase tracking-tight">{app.job?.title || "Job"}</h3>
                        <p className="text-sm text-slate-500 font-medium">{app.job?.companyId?.name || "Company"} • {app.job?.location}</p>
                        <div className="flex items-center gap-3 mt-3">
                          <span className={`text-[10px] font-black px-3 py-1 rounded-full border uppercase tracking-wider ${app.status === "Accepted" ? "text-emerald-700 bg-emerald-50 border-emerald-200" :
                        ...
                        {app.interview?.date && (
                          <div className="mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
                            <p className="text-sm font-black text-blue-800 flex items-center gap-2 font-black uppercase tracking-widest"><Calendar className="w-4 h-4" /> Interview Scheduled</p>
                            <div className="mt-2 space-y-1">
                              <p className="text-xs text-blue-700"><span className="font-bold">Date:</span> {new Date(app.interview.date).toLocaleDateString()}</p>
                              {app.interview.meetingLink && (
                                <a href={app.interview.meetingLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20">
                      ...
                      <div className="flex flex-col gap-2 items-end justify-start">
                        {app.resume && (
                          <button type="button" onClick={() => handleViewResume(app._id)} className="flex items-center w-full justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 border border-blue-200 px-4 py-2 rounded-lg hover:bg-blue-100 transition-colors">
                            <FileText className="w-4 h-4" /> View Resume
                          </button>
                        )}
                        {app.resume && !atsResults[app._id] && app.atsScore == null && (
                          <button type="button" onClick={() => handleATSScan(app._id)} disabled={scanningId === app._id} className="flex items-center w-full justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-purple-600 bg-purple-50 border border-purple-200 px-4 py-2 rounded-lg hover:bg-purple-100 transition-colors disabled:opacity-50">
                  ...
                  <div className="bg-white p-20 rounded-xl card-shadow text-center text-slate-400">No applications yet.</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl relative animate-fade-in overflow-hidden">
            ...
            <form onSubmit={handleApply} className="p-6 space-y-6">
              <div className="space-y-4">
                <p className="text-sm text-slate-600 font-medium italic">Apply to <span className="font-bold text-primary-600 underline Decoration-wavy">{selectedJob.companyId?.name}</span></p>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-800 uppercase tracking-widest">Resume Upload (PDF)</label>
                  <div className="relative border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer group">
                    ...
                    <input type="file" accept=".pdf" onChange={(e) => setResumeFile(e.target.files[0])} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                  </div>
                </div>
              </div>
              <button disabled={isApplying} type="submit" className="btn-primary w-full shadow-xl shadow-primary-600/20">
